...
function all_colors_customize_register_content( $wp_customize ) {

		function all_colors_sanitize_homepage_columns( $input ) {
			$valid = array(
				'1' => esc_html__( 'One', 'all-colors' ),
				'2' => esc_html__( 'Two', 'all-colors' ),
			);
 ...
 */
		$wp_customize->add_section( 'seos_content_section' , array(
			'title'       => __( 'Content Options', 'all-colors' ),
			'priority'    => 2,	
 		...
 		$wp_customize->add_setting( 'homepage_columns', array (
		    'default' => 3,
			'sanitize_callback' => 'all_colors_sanitize_homepage_columns',
		) );
				
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'homepage_columns', array(
			'label'    => __( 'Homepage Post Columns', 'all-colors' ),
		 ...
		 $wp_customize->add_control( 'content_max_width', array(
		  ...
		  'label' => __( 'Content max width', 'all-colors' ),
		  'input_attrs' => array(
			'min' => 1210,
			'max' => 2000,
			'step' => 1,
		 ...
		 $wp_customize->add_control( 'content_padding', array(
		  ...
		  'label' => __( 'Content Padding', 'all-colors' ),
		  'input_attrs' => array(
			'min' => 0,
			'max' => 100,
			'step' => 1,
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'hide_home_content', array(
			'label'    => __( 'Hide sidebar and content on home page', 'all-colors' ),
			'section'  => 'seos_content_section',
			'priority'    => 1,				
...
add_action( 'customize_register', 'all_colors_customize_register_content' );
...
function all_colors_content_styles () {
...
add_action( 'wp_enqueue_scripts', 'all_colors_content_styles' );