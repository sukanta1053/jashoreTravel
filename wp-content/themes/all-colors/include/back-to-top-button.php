...
add_action( 'customize_register', 'all_colors_back_to_top_customize_register' );

function all_colors_back_to_top_customize_register( $wp_customize ) {
...
***********************************************************************************/
		...
		$wp_customize->add_section( 'back_to_top' , array(
			'title'       => __( 'Back To Top Button Options', 'all-colors' ),
			'priority'   => 98,
		) );
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'activate_back_to_top', array(
			'label'    => __( 'Activate Back To Top Button', 'all-colors' ),
			'section'  => 'back_to_top',
		...
		'label' => __('Button Color', 'all-colors'),        
		'section' => 'back_to_top',
		...
		'label' => __('Button Hover Color', 'all-colors'),        
		'section' => 'back_to_top',
		...
		'label' => __('Button Background Color', 'all-colors'),        
		'section' => 'back_to_top',
		...
		'label' => __('Button Background Hover Color', 'all-colors'),        
		'section' => 'back_to_top',
...
add_action( 'wp_enqueue_scripts', 'all_colors_back_top_method' );	

function all_colors_back_top_method() {
...
function all_colors_to_top() {
    echo '<a id="totop" href="#"><span class="dashicons dashicons-arrow-up-alt2"></span></a>';
    }

    add_action( 'wp_head', 'all_colors_back_to_top_style' );
    function all_colors_back_to_top_style() {
    echo '<style>
    #totop {
		position: fixed;
		right: 40px;
